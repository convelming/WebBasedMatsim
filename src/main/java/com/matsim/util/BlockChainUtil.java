...
import com.matsim.bean.ActivityRelatedBlocks;
import com.matsim.bean.Block;
import com.matsim.bean.OdMatrixRelatedBlocks;
import com.matsim.bean.WorkSpace;
import com.matsim.bean.Edge;
import com.matsim.bean.WorkSpaceTopology;

import java.util.ArrayList;
import java.util.List;

/**
 * Created by MingLu on 2018/4/23.
 *
...
public class BlockChainUtil {
    ...
    public BlockChainUtil(WorkSpaceTopology workSpaceTopology){
...
//        this.matsimEdges = workSpaceTopology.getMatsimEdges();
    ...
    /**
     * This method get different subblocks in odmatrixRealtedBlocks or ActivityRelatedBlocks
     *
     * @param blocks list of block, can be OdmatrixRelatedBlocks or ActivityRelatedBlocks
     * @param blockType could be tripPurpose,mode,timer,person
     * @return
     */
    public List<Block> getSubTypeBlock(List<Block> blocks,String blockType){
        List<Block> tripPurposeList = new ArrayList<>();
        for (Block block:blocks){
            if(block.getType().equalsIgnoreCase(blockType)){
                tripPurposeList.add(block);
                //todo need to check the sequence of the these trip purposes
            }
        }
        return tripPurposeList;
    }
