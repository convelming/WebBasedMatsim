                ...
                for (int iCount = 0; iCount < tempCount; iCount++) {
                    ...
                    for (int j = 0; j < desNum; j++) {
                        ...
                        if (row.getCell(j * 8 + 6) != null) {
                            if (row.getCell(j * 8 + 6).getCellType() == CellType.STRING) {
                                tempDest.setDesiredMode(row.getCell(j * 8 + 6).getStringCellValue());
                            } else if (row.getCell(j * 8 + 6).getCellType() == CellType.NUMERIC) {
...
//                                    tempDest.setTripPurpose(row.getCell(j*8+7).getStringCellValue());
                            if (row.getCell(j * 8 + 7).getCellType() == CellType.STRING) {
                                tempDest.setTripPurpose(row.getCell(j * 8 + 7).getStringCellValue());
                            } else if (row.getCell(j * 8 + 7).getCellType() == CellType.NUMERIC) {
...
//                                    tempDest.setFacilityId(row.getCell(j*8+8).getStringCellValue());
                            if (row.getCell(j * 8 + 8).getCellType() == CellType.STRING) {
                                tempDest.setFacilityId(row.getCell(j * 8 + 8).getStringCellValue());
                            } else if (row.getCell(j * 8 + 8).getCellType() == CellType.NUMERIC) {
                                tempDest.setFacilityId(row.getCell(j * 8 + 8).getNumericCellValue() + "");
                            }
                        }
                        if (row.getCell(j * 8 + 9) != null) {
                            if (row.getCell(j * 8 + 9).getCellType() == CellType.STRING) {
                            ...
                            } else if (row.getCell(j * 8 + 9).getCellType() == CellType.NUMERIC && row.getCell(j * 8 + 9).getNumericCellValue() == 1.0) {
                                tempDest.setFacilityChangable(true);
                            } else {
                                tempDest.setFacilityChangable(false);
                            }
...
//                                    tempDest.setDesRegionId(row.getCell(j*8+10).getStringCellValue());
                            if (row.getCell(j * 8 + 10).getCellType() == CellType.STRING) {
                                tempDest.setDesRegionId(row.getCell(j * 8 + 10).getStringCellValue());
                            } else if (row.getCell(j * 8 + 10).getCellType() == CellType.NUMERIC) {
                    ...
                    if (row.getCell(j) != null) {
                        if ((int) row.getCell(j).getNumericCellValue() > 0) {
                            ...
                            for (int k = 0; k < tempOdCounts; k++) {
                                ...
                                } else if (header.getCell(j).getCellType() == CellType.NUMERIC) {
...
//        log.info( ods.toString() +" 531");
    ...
    public StringBuffer validateOriginalDataFromArrayCsv(String file) throws Exception {
        ...
        while ((line = br.readLine()) != null) { // skip header
            ...
            if (tempLine[0].isEmpty()) {
                error.add("row: " + (index + 1) + " col: " + 1 + ",id shouldn't be empty");
                break;
            }
            if (idCheck.contains(tempLine[0]) && Integer.parseInt(tempLine[1]) == 1) {
                error.add("row: " + (index + 1) + " col: " + 1 + ",there is some other agent using the same id");
            } else {
                idCheck.add(tempLine[0]);
            }
            if (tempLine[1].isEmpty()) {
                tempLine[1] = "1";
                error.add("row: " + (index + 1) + " col: " + 2 + ",id shouldn't be empty");
            }
            if (tempLine[2].isEmpty()) {
                tempLine[2] = "null";
                error.add("row: " + (index + 1) + " col: " + 3 + ",id shouldn't be empty");
            }
            if (!TimeFormatUtil.isHHMMSSformat(tempLine[2])) {
                tempLine[2] += "*";
                error.add("row: " + (index + 1) + " col: " + (2) + "; not HH:MM:SS format");
            }
            if (tempLine[3].isEmpty() && tempLine[4].isEmpty() && tempLine[5].isEmpty()) {
                tempLine[3] = "*";
                tempLine[4] += "*";
                tempLine[5] = "*";
                error.add("row: " + (index + 1) + " col: 4,5,6; regionId or coords should be filled");
            } else if (tempLine[3].isEmpty() && tempLine[4].isEmpty() && !tempLine[5].isEmpty()) {
                tempLine[3] = "null";
                tempLine[4] = "*";
                error.add("row: " + (index + 1) + " col:5; coordXEmpty");
            } else if (tempLine[3].isEmpty() && !tempLine[4].isEmpty() && tempLine[5].isEmpty()) {
                tempLine[3] = "null";
                tempLine[5] = "*";
                error.add("row: " + (index + 1) + " col:6; coordXEmpty");
            } else if (tempLine[3].isEmpty() && !tempLine[4].isEmpty() && !tempLine[5].isEmpty()) {
                tempLine[3] = "null";
            } else if (!tempLine[3].isEmpty() && tempLine[4].isEmpty() && tempLine[5].isEmpty()) {
                tempLine[4] = "null";
                ...
                tempLine[3] += "*";
                tempLine[5] += "*";
                error.add("row: " + (index + 1) + " col: 4,6; duplicatedInput");
            } else if (!tempLine[3].isEmpty() && !tempLine[4].isEmpty() && tempLine[5].isEmpty()) {
                tempLine[3] += "*";
                tempLine[4] += "*";
                ...
                tempLine[4] += "*";
                tempLine[5] += "*";
                error.add("row: " + (index + 1) + " col:4,5,6; duplicatedInput");
            }
            if (!isInteger(tempLine[1]) && !tempLine[1].isEmpty()) {
                tempLine[1] += "*";
                error.add("row: " + (index + 1) + " col: 2; int required");
            }
            if (!isDouble(tempLine[4]) && !tempLine[4].equalsIgnoreCase("null") && !tempLine[4].isEmpty()) {
                tempLine[4] += "*";
                error.add("row: " + (index + 1) + " col: 5; double required");
            }
            if (!isDouble(tempLine[5]) && !tempLine[5].equalsIgnoreCase("null") && !tempLine[5].isEmpty()) {
                tempLine[5] += "*";
                error.add("row: " + (index + 1) + " col: 6; double required");
            }
            if (isDouble(tempLine[4]) && (Double.parseDouble(tempLine[4]) > 180 || Double.parseDouble(tempLine[4]) <= 0)) {
                tempLine[4] += "*";
                error.add("row: " + (index + 1) + " col: 5; longitude range:0-180");
            }
            if (isDouble(tempLine[5]) && (Double.parseDouble(tempLine[5]) > 180 || Double.parseDouble(tempLine[5]) <= 0)) {
                tempLine[5] += "*";
                error.add("row: " + (index + 1) + " col: 6; longitude range:0-180");
            }
